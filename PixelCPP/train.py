...
def sample_imgs(model,images_to_log,x):
	print("Sample...")
	count = 4
	samples = model.sample(images_to_log)
	img = tf.cast((samples + 1.0) * 127.5, tf.uint8)
	...
	for i in range(count*count):
		plt.subplot(8, 8, i + 1 + 48)
		plt.imshow(img[i])
		plt.axis('off')

	fig.savefig("sample_%d.png" % (x))
    ...
    **kwargs
    ...
    with strategy.scope():
        ...
        def train_step(batch):
            ...
            per_replica_loss = strategy.run(step_fn, (batch,))
    ...
    sample_imgs(model,images_to_log,0)
    for epoch in trange(1, max_epoch + 1, initial=1):
        ...
        if epoch % log_images_every == 0:
            ...
            sample_imgs(model,images_to_log,epoch)
