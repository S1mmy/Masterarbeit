...
def plot_instance_score(id,preds: Dict,
                        target: np.ndarray,
                        labels: np.ndarray,
                        threshold: float,
    ...
    ax.legend()
    plt.savefig('plot_instance_score_%d.png' % (id))


def plot_feature_outlier_image(id,od_preds: Dict,
                               X: np.ndarray,
